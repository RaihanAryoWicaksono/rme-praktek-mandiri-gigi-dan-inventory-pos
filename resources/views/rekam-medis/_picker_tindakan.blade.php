{{-- Picker Tindakan dari Master Data --}}
<div x-data="{
    open: false,
    search: '',
    options: {{ Js::from($treatments->map(fn($t) => ['name' => $t->name, 'category' => $t->category])) }},
    picked: [],
    get filtered() {
        if (!this.search) return this.options;
        const q = this.search.toLowerCase();
        return this.options.filter(o => o.name.toLowerCase().includes(q) || o.category.toLowerCase().includes(q));
    },
    pick(item) {
    ...
    <div class="flex items-center justify-between mb-1">
        <label class="block text-sm font-medium text-gray-700">Tindakan <span class="text-red-500">*</span></label>
    ...
    <div x-show="picked.length > 0" class="flex flex-wrap gap-1 mb-2">
        <template x-for="item in picked" :key="item.name">
            <span class="inline-flex items-center gap-1 px-2 py-0.5 bg-teal-50 border border-teal-200 text-teal-700 rounded-full text-xs font-medium">
                <span x-text="item.name"></span>
                <button type="button" @click="remove(item.name)"
                        class="ml-0.5 text-teal-400 hover:text-red-500 leading-none">&times;</button>
            </span>
        </template>
    </div>
    ...
    <div x-show="open" x-cloak class="relative mb-2" style="z-index: 30;">
        <div class="absolute w-full bg-white border border-gray-200 rounded-lg shadow-lg">
            <div class="p-2 border-b border-gray-100">
                <input type="text" x-model="search" placeholder="Cari tindakan..." autofocus
            ...
            <ul class="max-h-44 overflow-y-auto py-1">
                <template x-for="item in filtered" :key="item.name">
                    <li @click="!picked.find(p => p.name === item.name) && pick(item)"
                        ...
                        <span class="text-xs text-gray-400 ml-2 shrink-0" x-text="item.category"></span>
                    </li>
                </template>
    ...
    <textarea name="tindakan" rows="3" required x-ref="ta"
              class="w-full px-3 py-2 text-sm border @error('tindakan') border-red-400 @else border-gray-300 @enderror rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 resize-none"
