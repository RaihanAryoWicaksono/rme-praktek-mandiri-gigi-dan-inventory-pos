<x-app-layout>
    ...
    <div class="max-w-4xl space-y-5">
        ...
        <form method="POST" action="{{ route('rekam-medis.update', $rekamMedis) }}" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
                ...
                <div>
                    ...
                    <textarea name="tindakan" rows="3" required
                              class="w-full px-3 py-2 text-sm border @error('tindakan') border-red-400 @else border-gray-300 @enderror rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 resize-none">{{ old('tindakan', $rekamMedis->tindakan) }}</textarea>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <textarea name="resep" rows="3"
                                  class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 resize-none">{{ old('resep', $rekamMedis->resep) }}</textarea>
                    </div>
                    <div>
