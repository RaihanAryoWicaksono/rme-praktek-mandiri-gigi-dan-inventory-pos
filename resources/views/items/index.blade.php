<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Master Item / Bahan</h2>
    </x-slot>
    ...
    <div class="space-y-5" x-data="{
        ...
        <div class="main-content-card p-6">
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Master Item / Bahan</h1>
                    <p class="text-sm text-gray-500 mt-1">{{ count($items) }} item terdaftar</p>
                </div>
            ...
            <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="w-full text-sm datatable" data-empty-message="Belum ada data item atau bahan.">
                            ...
                            @foreach($items as $item)
                                <tr class="hover:bg-gray-50 transition-colors">
                                    <td class="px-4 py-3">
                                        ...
                                        <p class="text-xs text-gray-400 mt-0.5">Min stok: {{ $item->min_stock_alert }} {{ $item->unit_use }}</p>
                                    </td>
                                    <td class="px-4 py-3 text-gray-500 capitalize">{{ $item->type }}</td>
                                    <td class="px-4 py-3 text-gray-600">
                                        {{ $item->unit_use }}
                                        @if($item->unit_purchase)
                                            <p class="text-xs text-gray-400">1 {{ $item->unit_purchase }} = {{ $item->conversion_factor }} {{ $item->unit_use }}</p>
                                        @endif
                                    </td>
                                    <td class="px-4 py-3">
                                        @if($item->is_active)
        ...
        <div x-show="showDeleteModal" x-cloak
            ...
            <div @click.away="showDeleteModal = false"
                ...
                <p class="text-sm text-gray-600 mb-6">Hapus item <span class="font-semibold text-red-600" x-text="itemName"></span>?</p>
